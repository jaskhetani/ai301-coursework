...
def grade_one(item_id: str, bundle_path: Path, skill: str, rubric: str,
              timeout: int, claude_cli: str) -> dict:
...
def main() -> int:
    ...
    # Resolve the executable up front. This is required for npm-installed
    # command shims on Windows (for example, claude.cmd), which CreateProcess
    # does not find when subprocess receives only the extensionless name.
    claude_cli = shutil.which("claude")
    if claude_cli is None:
        print("error: Claude Code CLI not found on PATH. Install it and verify "
              "that `claude --version` works before running this eval.",
              file=sys.stderr)
        return 2
    ...
    with concurrent.futures.ThreadPoolExecutor(a.workers) as pool:
        futs = {pool.submit(grade_one, it["id"],
                            ...
                            skill, rubric, a.timeout, claude_cli): it["id"]
                for it in items}
